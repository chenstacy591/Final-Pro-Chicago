...
March 2020, when Chicago first issued 
stay-at-home order",
...
domvio_phase<-as.data.frame(table(domvio_mut$PHASE, domvio_mut$isdomviolence))
...
domvio_phase <-domvio_phase %>% 
  mutate(pandemic_phase_days = case_when(Var1 == "before_covid" ~ 198 ,
                                         Var1 == "before-lockdown" ~ 58,
                                         Var1 == "lockdown" ~ 40,
                                         Var1 == "phase 2" ~ 33,
                                         Var1 == "phase 3" ~ 23,
                                         Var1 == "phase 4" ~ 12
...
ggplot(data = domvio_phase, mapping = aes(x = Var1, y = PERDAYDOMCASE)) + geom_boxplot() +
 labs(title = "dom violence cases rate appears to increase over phases",
       x = "phase",
        y = "avg domestic violence cases/day") + expand_limits(x = 0, y = 0)
...
ggplot(data = domvio_perday, mapping = aes(x = Var1, y = DOMVIOCASES, colour = period_indicator, group=1)) + geom_line()+ geom_point() +
  labs(title = "daily domvio cases over time shows domestic crime rates over different phases are not constant",
...
ggplot(data = domvio_perday, mapping = aes(x = Var1, y = TOTCASES, colour = period_indicator, group=1))+ geom_line() + geom_point() +
  labs(title = "daily crime cases over time shows crime rates over the different phases aren't constant /n
       note the increased crimes around the time of George Floyd's death",
       x = "Year and Month (YYYYMMDD)",
       y = "daily cases")
```
...
H~0 : Domestic violence cases in Chicago is unrelated to the 3 periods of the pandemic.
H~1 : Domestic violence cases in Chicago is related to the 3 periods of the pandemic.

*The 6 phases of the pandemic is divided into 3 periods: before-lockdown (before COVID and before lockdown), during lockdown (lockdown and phase 2), and post-lockdown (phases 3 and 4)
```{r chisquared test}
table(domvio_mut$isdomviolence, domvio_mut$PHASE)
chisq.test(table(domvio_mut$isdomviolence, domvio_mut$PHASE))
```

Under the null hypothesis, our test statistic has a chi-square distribution with 2 degrees of freedom. We performed the test and obtained a chi-square value of 247.63, which corresponds to a p-value of < 0.001. Thus, at an $\alpha$ = 0.05 significance level, we reject the null hypothesis; there is sufficient evidence to suggest that domestic violence cases in Chicago is related to the 3 periods of the pandemic.
...
```
We found that all three pairwise difference in proportions are significant at the adjusted $\alpha$ = 0.05/3 significance level. There is sufficient evidence to suggest that the proportion of crime in Chicago related to domestic violence is different for each period of the pandemic.

#### Regression Analysis
(BLAH BLAH copy from doc)
...
$\hat{\beta}_0$ + $\hat{\beta}_1$ * (PHASE == lockdown) + $\hat{\beta}_2$ * (PHASE == phase 2) + $\hat{\beta}_3$ * (PHASE == phase 3) + $\hat{\beta}_4$ * (PHASE == phase 4) + $\hat{\beta}_5$ * (PHASE == before-lockdown) + $\hat{\beta}_6$ * (is9_5)
