...
domvio<-domvio %>%
  mutate(MONTH = as.numeric(factor(substring(DATE..OF.OCCURRENCE, 0, 2)))) %>% 
  mutate(DAY = as.numeric(factor(substring(DATE..OF.OCCURRENCE, 4,5)))) %>% 
  mutate(YEAR = as.numeric(factor(substring(DATE..OF.OCCURRENCE, 7,10))))
